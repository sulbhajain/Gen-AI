...
This server registers calculator, file operations, weather, joke generation,
and python code generation tools and communicates via stdio transport.
"""
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name=joke_tool["name"],
            description=joke_tool["description"],
            inputSchema=joke_tool["inputSchema"]
        ),
        Tool(
            name=codegen_tool["name"],
            description=codegen_tool["description"],
            inputSchema=codegen_tool["inputSchema"]
        ),
    ]
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    elif name == "generate_joke":
        result = await handle_generate_joke(arguments)
    elif name == "generate_python_code":
        result = await handle_generate_python_code(arguments)
    else:
...
async def main():
    ...
    print("Available tools: calculator, read_file, write_file, list_files, get_weather, generate_joke, generate_python_code", file=sys.stderr)
