...
3. **Install dependencies:**
   ...
   ```
   > Note: `chromadb` is pinned below 0.5 to avoid the `onnxruntime` dependency,
   > which may not have wheels for some Python versions.
...
| `generate_joke` | Generate a joke about a topic |
| `generate_python_code` | Generate a Python snippet from a prompt |
...
- **Note**: Preferences/facts are only stored when explicitly stated by the user

### Episodic Memory
- **What**: Past task executions and experiences
- **How**: ChromaDB vector store with hybrid search (semantic + keyword)
- **Hybrid search behavior**: A keyword filter is applied first, with a fallback to semantic-only retrieval when no keyword matches are found
