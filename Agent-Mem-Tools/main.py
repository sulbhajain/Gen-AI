...
def print_banner():
    ...
    print("  • generate_joke- Generate a light joke on a topic")
    print("  • generate_code- Generate Python code snippets")
    print()
    print("Commands:")
...
def show_memory(user_id: str):
    ...
    if episodes:
        ...
        for ep in episodes[:5]:
            ...
            if ep.get("metadata"):
                meta_preview = ", ".join(
                    f"{k}: {v}" for k, v in ep["metadata"].items()
                )
                if meta_preview:
...
def clear_memory(user_id: str):
    ...
    # Clear short-term memory (checkpointer)
    reset_checkpointer()
